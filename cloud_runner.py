...
# ── Fetch prices using markets endpoint (has 7d data) ───────────────────
def fetch_prices():
    all_ids = list(CG.values()) + list(WATCHLIST.values())
    # Use /coins/markets which reliably returns 7d change
    ...
    # Also get SGD prices
    sgd = get("https://api.coingecko.com/api/v3/simple/price", {
        "ids": ",".join(all_ids),
        "vs_currencies": "sgd",
    }, delay=1.5)
    ...
    for coin in d:
        cid = coin["id"]
        sgd_price = 0
        if sgd and cid in sgd:
            sgd_price = sgd[cid].get("sgd", 0)
        by_id[cid] = {
            "sgd":  sgd_price,
            "usd":  coin.get("current_price", 0),
            "h24":  round(coin.get("price_change_percentage_24h") or 0, 2),
            "d7":   round(coin.get("price_change_percentage_7d_in_currency") or 0, 2),
